...
- A local OTLP collector (Jaeger all-in-one) is started by `./scripts/run_jaeger.sh` with Podman (`compose.yaml` / `podman run`, loopback `:4317` / UI `:16686`). Distributed traces are produced by the governed proxy; the comparison script’s labeled stdout is the human-readable report.
