...
- A local OTLP collector (for example Jaeger all-in-one) is already running. The demo does not start the collector. Distributed traces are produced by the governed proxy; the comparison script’s labeled stdout is the human-readable report.
